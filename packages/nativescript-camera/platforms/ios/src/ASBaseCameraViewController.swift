...
@objc public typealias ASCamera = ASBaseCameraViewController
@objc public typealias ASCameraLocation = AVCaptureDevice.Position
...
@objc open class ASBaseCameraViewController: UIViewController {
    ...
    private(set) var captureView = ASCameraView()
...
extension ASBaseCameraViewController {
...
@objc public extension ASBaseCameraViewController {
    ...
    func startRecording() {
        assert(Thread.isMainThread, "[ASCamera]: This function -startRecording must be called on the main thread.")
    ...
    func stopRecording() {
        assert(Thread.isMainThread, "[ASCamera]: This function -stopRecording must be called on the main thread.")
        
        self.executeAsync { [weak self] in
            guard let self = self else { return }
            assert(
                self.shouldStartWritingSession, "[ASCamera]: Called stopRecording() while video is not being recorded")
    ...
    func cancelRecording() {
        assert(Thread.isMainThread, "[ASCamera]: This function -cancelRecording must be called on the main thread.")
        
        self.executeAsync { [weak self] in
            guard let self = self else { return }
            assert(
                self.shouldStartWritingSession, "[ASCamera]: Called cancelRecording() while video is not being recorded")
            guard let assetWriter = self.assetWriter else { return }
            self.shouldStartWritingSession = false
            self.didStartWritingSession = false
            self.frameCount = 0
...
@objc internal extension ASBaseCameraViewController {
    //
...
extension ASBaseCameraViewController: AVCaptureVideoDataOutputSampleBufferDelegate,
