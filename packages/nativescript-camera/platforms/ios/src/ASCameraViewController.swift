...
@objc extension ASCameraViewController {
  private func setFlash(to torchMode: AVCaptureDevice.TorchMode) {
    guard let device = self.captureDevice,
      device.hasTorch, !self.isFlashLocked
    else { return }
    DispatchQueue.main.async {
      do {
        ...
        NSLog("[asCamera]: Could not configure device Torch: \(error)")
      }
    }
  }
...
@objc extension ASCameraViewController {
  ...
  private func addGestureRecognizers() {
    ...
    self.singleTapGestureRecognizer = UITapGestureRecognizer(
      target: self, action: #selector(self.handleSingleTap(_:)))
    ...
    self.doubleTapGestureRecognizer = UITapGestureRecognizer(
      target: self, action: #selector(self.doubleTapGesture(_:)))
