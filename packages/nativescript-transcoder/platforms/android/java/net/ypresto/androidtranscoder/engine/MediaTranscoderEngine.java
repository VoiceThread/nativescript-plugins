...
public class MediaTranscoderEngine {
     ...
     * Audio data will not be transcoded; original stream will be wrote to output
     * file.
     *
    ...
    private void setupMetadata() throws IOException {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            String locationString = mediaMetadataRetriever
    ...
    private void setupTrackTranscoders(MediaFormatStrategy formatStrategy) {
        ...
        final boolean isAudioTrackExists = trackResult.mAudioTrackIndex != MediaExtractorUtils.TrackResult.TRACK_NOT_FOUND;
        MediaFormat audioOutputFormat = isAudioTrackExists
                ? formatStrategy.createAudioOutputFormat(trackResult.mAudioTrackFormat)
                : null;
        ...
        QueuedMuxer queuedMuxer = new QueuedMuxer(mMuxer, isAudioTrackExists, new QueuedMuxer.Listener() {
            @Override
        ...
        if (isAudioTrackExists) {
            if (audioOutputFormat == null) {
                ...
                mAudioTrackTranscoder = new AudioTrackTranscoder(mExtractor, trackResult.mAudioTrackIndex,
                        audioOutputFormat, queuedMuxer);
            }
    ...
    private void runPipelines() throws InterruptedException {
        ...
        while (!(mVideoTrackTranscoder.isFinished() &&
            ...
            boolean stepped = mVideoTrackTranscoder.stepPipeline()
                    || (mAudioTrackTranscoder != null && mAudioTrackTranscoder.stepPipeline());
            loopCount++;
            if (mDurationUs > 0 && loopCount % PROGRESS_INTERVAL_STEPS == 0) {
                ...
                double audioProgress = mAudioTrackTranscoder == null ? 0
                        : mAudioTrackTranscoder.isFinished() ? 1.0
                                : Math.min(1.0,
