...
export class DemoModel extends DemoSharedNativescriptTranscoder {
  ...
  constructor() {
    ...
    this.transcoder.on(NativescriptTranscoder.TRANSCODING_STARTED, (payload: MessageData) => {
    ...
    this.transcoder.on(NativescriptTranscoder.TRANSCODING_ERROR, (payload: MessageData) => {
      executeOnMainThread(() => {
        // IMPORTANT! You'll have to wrap any UI updates in `executeOnMainThread` for iOS as the events are emitted from a different thread
        const progressBar = Frame.topmost().currentPage.getViewById('transcodingProgress') as Progress;
        progressBar.value = 0;
    ...
    this.transcoder.on(NativescriptTranscoder.TRANSCODING_COMPLETE, (payload: MessageData) => {
      executeOnMainThread(() => {
        // IMPORTANT! You'll have to wrap any UI updates in `executeOnMainThread` for iOS as the events are emitted from a different thread
  ...
  convertAudio(inputPath: string) {
    ...
    this.transcoder
      ...
      .then(transcodedFile => {
        ...
        const outputFile = File.fromPath(transcodedFile.path);
  ...
  processVideo(height: number, frameRate?: number) {
    ...
    this.transcoder
      .transcode(this.pickedFile.path, tempPath, {
        ...
        force: false, //set this to allow transcoding to higher resolutions
      })
      .then(transcodedFile => {
        if (!transcodedFile) {
          console.error('transcode did not return a file, error occurred!');
